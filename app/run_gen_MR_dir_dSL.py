...
# -*- coding: utf-8 -*-
"""
Created on Thu May  1 14:35:16 2025

@author: dexter

A script that generate signals for dynamical stop-loss backtest

1) Change SL at different time. Dynamically adjust based on current price
2) If the trade hasn't happen until some time, cancel the order
3) Based on the previous days of volatility, change the target entry price
   (faster drop, expect inversion, buy lower quantile)
4) Slow TP level rises
5) Defense mode or Profit mode
"""
...
def loop_signal(strategy: type[Strategy], 
                ...
                TE_time: tuple[datetime.datetime] = (), # A pair of datetime in a tuple
                TP_time: tuple[datetime.datetime] = (), # A pair of datetime in a tuple
                dSL_time: list[tuple[float]] = [(),(),(),()], # A list of pairs of datetime in a tuple
                dSL: list[tuple[float]] = [], # A list of float in the form of quant distance from the SL set in buy_range or sell_range
                **kwargs) -> pd.DataFrame: 
    ...
    #make bucket
    bucket = book.make_bucket(keyword='argus_exact_4dSL') # 'argus_exact_mode')#
    ...
    for i in np.arange(start_index,end_index+1): 
        ...
        if len(APCs_this_date) == 0:
            ...
            # Make dSL information
            quant_list = np.arange(0.0025, 0.9975, 0.0025)
            curve_this_date_spline = mfunc.generic_spline(quant_list, 
                                                          curve_this_date)
            
            TE_price = strategy_output['data'][-4]
            TP_price = strategy_output['data'][-3]
            
            if strategy_output['direction'] == 'Buy':
                SL_price_list = [curve_this_date_spline(buy_range[2]+ele) 
                                 for ele in dSL]
            elif strategy_output['direction'] == 'Sell':
                SL_price_list = [curve_this_date_spline(sell_range[2]-ele) 
                                 for ele in dSL]
            else: 
                SL_price_list = ['NA']* len(dSL)
            ...
            dSL_info = [TE_price] + \
                       ...
                       [TP_time[0]] + [TP_time[1]] +\
                       SL_price_list +\
                       [dSL_time[0][0]] + [dSL_time[0][1]] + \
                       [dSL_time[1][0]] + [dSL_time[1][1]] + \
                       [dSL_time[2][0]] + [dSL_time[2][1]] + \
                       [dSL_time[3][0]] + [dSL_time[3][1]]
                       
            print(dSL_info)
            # put all the data in a singular list
            data = [forecast_date, price_code] + \
                    [strategy_output['direction']] + \
                    static_info +  strategy_output['data'] + dSL_info
...
def run_gen_MR_signals_preloaded(strategy: type[Strategy], 
                                 ...
                                 TP_time: tuple[datetime.datetime] = (), # A pair of datetime in a tuple
                                 dSL_time: list[tuple[float]] = [(),(),(),()], # A list of pairs of datetime in a tuple
                                 dSL: list[tuple[float]] = [], # A list of float in the form of quant distance from the entry
                                  **kwargs) -> pd.DataFrame:
...
def run_gen_signal_bulk(strategy: type[Strategy], 
                        ...
                        TE_time = (), # A pair of datetime in a tuple
                        TP_time = (), # A pair of datetime in a tuple
                        dSL_time = [(),(),(),()], # A list of pairs of datetime in a tuple
                        dSL = [], # A list of float in the form of quant distance from the entry
                        runtype: str = 'list', 
                        **kwargs) -> None:
    default_kwargs = DEFAULT_KWARGS
    kwargs = dict(default_kwargs,**kwargs)
...
if __name__ == "__main__":
    ...
    buy_range = ([0.25,0.4],[0.6,0.75],0.05) # (-0.1,0.1,-0.45)
