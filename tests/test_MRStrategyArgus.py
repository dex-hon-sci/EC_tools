...
from EC_tools.strategy2 import MRStrategyArgus, SignalStatus
...
class TestSingleRun():
    ...
    def test_MRStrategyArgus_gen_data(self)->None:
        ...
        qunatile_info_answer = mfunc.generic_spline(
            quant_list, self.curve_this_date)([0.25,0.4,0.6,0.75])
        ...
        print(strategy_info_answer, rolling_avg_lag)
        #print(strategy_info['rollingaverage'] - rolling_avg_lag, 
        #      (strategy_info['rollingaverage'] - rolling_avg_lag) ==0)
    ...
    def test_MRStrategyArgus_rund_cond(self)->None:
        ...
        # Run the programme.
        strategy_info, qunatile_info = \
            MRStrategyArgus(self.curve_this_date).gen_data(history_data_lag5, 
                                                           apc_curve_lag5)
        
        MR = MRStrategyArgus(self.curve_this_date)
        
        direction, cond_info = MR.run_cond(
                                    strategy_info, self.open_price, 
                                    total_lag_days = 2, 
                                     apc_mid_Q = 0.5, 
        ...
        print(direction, cond_info)
        assert cond_info == [2 , 5, 'Sell', 'Sell']
        assert direction == SignalStatus().SELL
        
    def test_MRStrategyArgus_set_EES(self)->None:
        apc_curve_lag5, history_data_lag5 = read.extract_lag_data(self.signal_data, 
                                                                  self.history_data_daily, 
                                                                  self.this_date)
        MR = MRStrategyArgus(self.curve_this_date)
...
#TestSingleRun(DATE_STR).test_MRStrategyArgus_rund_cond()
TestSingleRun(DATE_STR).test_MRStrategyArgus_set_EES()
