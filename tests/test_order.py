...
from EC_tools.trade.order import OrderStatus, Order, ExecuteOrder
from EC_tools.portfolio import Portfolio

def test_OrderStatus()->None:
    Pos = OrderStatus
...
def test_Order_input_valid()-> None:
    # Initilise portfolio
    P1 = Portfolio()
    ...
    # 
    PP = Order( USD, A, 1000/50,portfolio=P1, 
    ...
    assert PP.status == OrderStatus.PENDING
...
def test_Order_input_invalid()-> None:
    # Initilise portfolio
    P1 = Portfolio()
...
def test_position_price_setter()-> None:
    ...
    PP = Order(USD, A, USD['quantity']/ A['quantity'], portfolio=P1, 
                    open_time = datetime.datetime(2020,1,1))    
...
def test_Execute_fill_pos()-> None:
    ...
    PP = Order(USD_exchange, A, 10.0, portfolio=P1, 
    ...
    #new_state = ExecuteOrder(PP).open_pos(open_time = datetime.datetime(2020,1,3))
    ...
    ExecuteOrder(PP).fill_pos(fill_time = datetime.datetime(2020,1,4))
    print(PP.status)
    print('pool_asset', P1.pool_asset)
    
    assert PP.status == OrderStatus.FILLED
...
def test_Execute_cancel_pos()->None:
    ...
    ExecuteOrder(PP).cancel_pos(void_time= datetime.datetime(2020,1,3))
...
def test_ErrorMsg_auto_adjust_off_invalid_price():
    ...
    A = {'name':"CL24N", 'quantity':50,  'unit':'contracts', 
         'asset_type':'future', 'misc':{}}
    P1.add(USD, datetime= datetime.datetime(2019,12,31))
    PP = Order(USD, A, USD['quantity']/ A['quantity'], 
...
def test_ErrorMsg_invalid_portfolio()->None:
    ...
    # A Order can belong to no Portfolio but it cannot be executed.
    with pytest.raises(Exception):
        ExecuteOrder(PP)
...
def test_ErrorMsg_insufficient_asset_fill_pos() -> None:
    ...
    PP = Order(USD_trade, A, USD_trade['quantity']/ A['quantity'], 
                  portfolio=P1, open_time = datetime.datetime(2020,1,1))        

    with pytest.raises(Exception):
        ExecuteOrder(PP).fill_pos()
...
def test_ErrorMsg_fill_pos_error_msg()-> None:
    ...
    PP = Order(USD, A, 1.0, portfolio=P1, 
